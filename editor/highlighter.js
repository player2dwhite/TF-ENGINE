...
(function (TF) {
    ...
    function analyze(text) {
        ...
        tokens.forEach(function (t) {
                ...
                if (CONTAINMENT[t.tag]) {
                    ...
                    if (!parent || parent.tag !== CONTAINMENT[t.tag]) {
                        out.push(diag("error", t.startLine, t.startLine, t.startCol, t.endCol,
                            "tf-nesting", "<" + t.tag + "> it must go inside <" + CONTAINMENT[t.tag] + ">."));
                    }
                }

                if (t.tag.indexOf("tf-") === 0 && KNOWN_COMPONENTS.indexOf(t.tag) === -1) {
                    out.push(diag("error", t.startLine, t.startLine, t.startCol, t.endCol,
                        "tf-unknown-component", "'<" + t.tag + ">' It is not a valid TF-Engine component."));
                }
                return;
            }
            // closing
            if (!stack.length) {
                out.push(diag("error", t.startLine, t.startLine, t.startCol, t.endCol,
                    "tf-unexpected-close", "Unexpected closing label '</" + t.tag + ">' without prior opening."));
                return;
            }
            var open = stack[stack.length - 1];
            if (open.tag !== t.tag) {
                out.push(diag("error", t.startLine, t.startLine, t.startCol, t.endCol,
                    "tf-mismatched-close", "it was expected '</" + open.tag + ">' but it was found '</" + t.tag + ">'."));
            ...
            if (matchIndex !== -1) {
                ...
                if (betweenIsEmpty && KNOWN_COMPONENTS.indexOf(t.tag) !== -1) {
                    out.push(diag("warning", openTag.startLine, t.endLine, openTag.startCol, t.endCol,
                        "tf-empty-component", "<" + t.tag + "> it's empty"));
                }
        ...
        stack.forEach(function (open) {
            out.push(diag("error", open.startLine, open.startLine, open.startCol, open.endCol,
                "tf-unclosed", "The closure label is missing for '<" + open.tag + ">'."));
        });
        ...
        tokens.forEach(function (t) {
            ...
            if (raw.trim() === "") {
                out.push(diag("warning", t.startLine, t.startLine, t.startCol, t.endCol,
                    "tf-empty-attr", "El attribute class it's empty."));
                return;
            }
            var seen = {};
            raw.split(/\s+/).filter(Boolean).forEach(function (cls) {
                ...
                if (cls.indexOf("tf-") === 0 && !isKnownClass(cls)) {
                    out.push(diag("warning", t.startLine, t.startLine, t.startCol, t.endCol,
                        "tf-unknown-class", "Class '" + cls + "'not recognized in TF-Engine."));
                }
            });
        });
        ...
        tokens.forEach(function (t) {
            ...
            Object.keys(t.attrs).forEach(function (name) {
                if (t.attrs[name] === "" && name !== "class") {
                    out.push(diag("warning", t.startLine, t.startLine, t.startCol, t.endCol,
                        "tf-empty-attr", "The attribute '" + name + "' it's empty"));
                }
            });
            if (t.attrs.style && t.attrs.style.length > 300) {
                out.push(diag("warning", t.startLine, t.startLine, t.startCol, t.endCol,
                    "tf-large-inline-style", "Very long inline style (" + t.attrs.style.length + " characteres)."));
            }
            if (t.attrs.style) {
                var declared = {};
                t.attrs.style.split(";").forEach(function (decl) {
                    ...
                    if (m) {
                        if (declared[m[1]]) {
                            out.push(diag("warning", t.startLine, t.startLine, t.startCol, t.endCol,
                                "tf-duplicate-variable", "The variable '" + m[1] + "' It is defined more than once in this style."));
                        }
                        declared[m[1]] = true;
                    }
                });
        ...
        tokens.forEach(function (t) {
            ...
            if (t.attrs["data-theme"]) {
                out.push(diag("info", t.startLine, t.startLine, t.startCol, t.endCol,
                    "tf-theme-detected", "Theme dectected: " + t.attrs["data-theme"]));
            }
            var cls = t.attrs.class || "";
            var themeMatch = /\btf-theme-(\S+)\b/.exec(cls);
            if (themeMatch) {
                out.push(diag("info", t.startLine, t.startLine, t.startCol, t.endCol,
                    "tf-theme-detected", "Theme dectected: " + themeMatch[1]));
            }
            ["tf-fade", "tf-slide", "tf-zoom", "tf-pulse"].forEach(function (animClass) {
                if ((" " + cls + " ").indexOf(" " + animClass + " ") !== -1) {
                    out.push(diag("info", t.startLine, t.startLine, t.startCol, t.endCol,
                        "tf-animation-detected", "Animation dectected: " + animClass));
                }
            });
        });
    ...
    function checkAttrs(t, out) {
        ...
        Object.keys(rules).forEach(function (attrName) {
            ...
            if (rules[attrName].indexOf(value) === -1) {
                out.push(diag("error", t.startLine, t.startLine, t.startCol, t.endCol,
                    "tf-invalid-attribute", "'" + attrName + "=\"" + value + "\"' is not valid in <" + t.tag + ">."));
            }
        });
    }
    ...
    TF.editor.diagnostics = {
        ...
        getDiagnostics: function () { return results.slice(); },
        getErrors: function () { return results.filter(function (d) { return d.severity === "error"; }); },
        getWarnings: function () { return results.filter(function (d) { return d.severity === "warning"; }); },
        getInfo: function () { return results.filter(function (d) { return d.severity === "info"; }); },
        init: function () {
